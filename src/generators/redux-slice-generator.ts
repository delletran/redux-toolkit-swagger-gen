...
const generateSliceFileContent = (sliceName: string, schemaName: string, uniqueImports: any[], interfaceName: string): string => {
...
const updateConstantsFile = (tempConstantsFilePath: string, sliceNames: string[]): void => {
  const constantsFileContent = fs.readFileSync(tempConstantsFilePath, 'utf-8');

  // Extract the FORM_SLICE content
  const formSliceMatch = constantsFileContent.match(/FORM_SLICE = {([^}]+)} as const/);
  if (!formSliceMatch) {
  ...
  sliceNames.forEach(sliceName => {
    const formSliceRegex = new RegExp(`\\s*${sliceName.toUpperCase()}: '${sliceName}-form-slice',`, 'g');
    formSliceContent = formSliceContent.replace(formSliceRegex, '');
  });

  // Add new slice constants
  const newEntries = sliceNames.map(sliceName => `  ${sliceName.toUpperCase()}: '${sliceName}-form-slice',`).join('\n');
  ...
  const finalContent = constantsFileContent.replace(/FORM_SLICE = {[^}]+} as const/, `FORM_SLICE = {\n${sortedFormSliceEntries}\n} as const`);

  fs.writeFileSync(tempConstantsFilePath, finalContent);
};

export const generateReduxSlices = async (definitions: any, outputDir: string): Promise<void> => {
  ...
  const tempConstantsFilePath = path.resolve(outputDir, 'redux/constants.temp.ts');
  ...
  for (const [name, schema] of Object.entries(definitions)) {
    ...
    const sliceContent = generateSliceFileContent(sliceName, name, uniqueImports, interfaceName);
  ...
  fs.copyFileSync(tempConstantsFilePath, path.resolve(outputDir, 'redux/constants.ts'));
