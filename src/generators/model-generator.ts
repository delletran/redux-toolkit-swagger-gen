...
const modelTemplate = fs.readFileSync(path.resolve(__dirname, '../templates/modelTemplate.mustache'), 'utf-8');
...
const getZodType = (property: any, nestedModels: Set<string>): string => {
  ...
  switch (baseType) {
    case 'string':
      zodType = 'z.string()';
      if (property.format === 'date') zodType += '.regex(/^\\d{4}-\\d{2}-\\d{2}$/)';
      if (property.format === 'uri') zodType += '.url()';
      if (property.format === 'email') zodType += '.email()';
      if (property.enum) zodType = `z.enum([${property.enum.map((e: string) => `'${e}'`).join(', ')}])`;
...
const generateProperties = (properties: any, required: string[] = [], nestedModels: Set<string>): any[] => {
...
const generateModelFileContent = (modelName: string, schema: any): string => {
  ...
  const properties = generateProperties(schema.properties, schema.required, nestedModels);
...
export const generateModels = async (definitions: any, outputDir: string): Promise<void> => {
