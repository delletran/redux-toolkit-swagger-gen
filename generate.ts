...
[reduxDir, sliceDir, schemaDir, thunkDir, path.join(reduxDir, 'helper'), constantsDir].forEach(dir => {
...
const generateServices = (paths: ExtendedPathType[], definitions: DefinitionType, generateIndex = false): void => {
  ...
  for (const [route, methods] of Object.entries(paths)) {
    ...
    const method = (['get', 'put', 'patch', 'delete'] as MethodType[]).find(m => methods[m]);
  ...
  if (generateIndex) {
    ['services', 'thunks'].forEach(type => {
      ...
      fs.writeFileSync(path.join(type === 'services' ? outputDir : thunkDir, 'index.ts'), indexContent);
    });
  }
};
...
const main = async () => {
  ...
  const reduxFiles = ['response.ts', 'types.ts', 'query.ts', 'actions.ts', 'helper/array.ts'];

  reduxGlobalFiles.forEach(file => {
    fs.copyFileSync(path.resolve(__dirname, `./redux/${file}`), path.join(sliceDir, file));
  });
  reduxFiles.forEach(file => {
    fs.copyFileSync(path.resolve(__dirname, `./redux/${file}`), path.join(reduxDir, file));
  });
